...
func (c *cache[K, V]) AddWithTTL(k K, x V, d time.Duration) {
	// "Inlining" of set
	var e int64
...
func (c *cache[K, V]) setWithTTL(k K, x V, d time.Duration) {
	var e int64
	if d == DefaultExpiration {
		d = c.defaultExpiration
	}
